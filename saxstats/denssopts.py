...
def parse_arguments(parser):
    ...
    parser.add_argument("--ligand_mask_mode", default='cubic', type=str, help='''Mask shape: 
cubic, sphere = use the ligand center and box size to construct search volume.
template = dilate a guess mrc from a pdb by 2 voxels. 
UB = dilate from a support generated by PDB (default 3 voxels; change this behavior by setting --ligand_box_volume).
UB_erode = dilate from a PDB (default 3 voxels; use above to change behavior) and zero the guess electron density in the outermost 1 voxel)
bbox = bounding box from a provided pdb, volume can be adjusted similarly
bsphere = bounding sphere from a provided pdb, volume can be adjusted similarly''')
    parser.add_argument("--ligand_box_volume", default=0, type=float, help="Desired minimum search volume. Support from ligand pdb will be dilated until this volume is reached (only works with ligand_mask_mode UB / UB_erode).")
    parser.add_argument("--ligand_template", default=None, type=str, help="Reference mask file name (maybe from a known ligand). Supply an .mrc file for ligand_mask_mode = template, and a .pdb file for ligand_mask_mode = UB/UB_erode")
    ...
    parser.add_argument("--refine_mode", default='pip', type=str, help='''[real / pip / switch / er / hio / refine]. 
...
er = use error reduction all the way
hio = use hybrid input-output all the way
...
''')
    ...
    parser.add_argument("--alternate_ER_HIO", dest="alternate_ER_HIO", action="store_true", help='Whether to use hybrid input-output')
    parser.set_defaults(alternate_ER_HIO=False)
    parser.add_argument("--alternate_period", default=100, type=int, help="If alternating ER and HIO, the period")
    ...
    if args.ligand_template is not None: # ligand_ref is a rho map, must have the same shape as ref_rho
        ...
        elif args.ligand_mask_mode in ['UB', 'UB_erode', 'bbox', 'bsphere']:
